...
export type Property =
	...
	| { optional?: boolean; type: "number"; validIntegerRanges?: [number, number][]; regex?: RegExp } // valid?: number[] }
...
export namespace Interface {
	export function create(value: any, name?: string): Interface | undefined {
		return typeof value == "string"
			? {
					...
					regex: {},
					ignore: [],
					properties: { optional: false, type: "string" },
			  }
			: undefined
	}
	...
	export function generateTypeguardPart(prefix: string, property: Property | NamedProperty): string {
		...
		switch (property.type) {
			case "array":
				result = `Array.isArray(${fullName} && ${property.value
					.map(p => generateTypeguardPart(fullName, p))
					.join(" && ")}`
				break
			case "object":
				result = `${
					property.typeguardFunctionName
						? property.typeguardFunctionName + "(" + fullName + ")"
						: `typeof ${fullName} == "object" && ${fullName}`
				} && ${property.value.map(p => generateTypeguardPart(fullName, p)).join(" && ")}`
				break
			case "union":
			...
			case "boolean":
				...
				break
			case "string":
				result = `typeof ${fullName} == "string"${
					property.regex ? ` && ${property.regex.source}.test(${fullName})` : ""
				}`
				break
			case "number":
				result = `typeof ${fullName} == "number"${
					property.regex ? ` && ${property.regex.source}.test(${fullName}.toString())` : ""
				}${
					property.validIntegerRanges
						? ` && ${property.validIntegerRanges
								.map(range => `${fullName} > ${range[0]} && ${fullName} < ${range[1]}`)
								.join(" && ")}`
						: ""
				}`
				break
